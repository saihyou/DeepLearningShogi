...
inline float make_result(const uint8_t result, const Color color) {
  ...
  if (gameResult == Draw) return 0.5f;
...
void __hcpe_decode_with_value(const size_t len, char *ndhcpe, char *ndfeatures1,
                              char *ndfeatures2, char *ndmove, char *ndresult,
                              char *ndvalue, char *ndlegal_moves) {
  HuffmanCodedPosAndEval *hcpe =
      reinterpret_cast<HuffmanCodedPosAndEval *>(ndhcpe);
...
void __hcpe2_decode_with_value(const size_t len, char *ndhcpe2,
                               char *ndfeatures1, char *ndfeatures2,
                               char *ndmove, char *ndresult, char *ndvalue,
                               char *ndaux) {
  HuffmanCodedPosAndEval2 *hcpe =
      reinterpret_cast<HuffmanCodedPosAndEval2 *>(ndhcpe2);
  features1_t *features1 = reinterpret_cast<features1_t *>(ndfeatures1);
  features2_t *features2 = reinterpret_cast<features2_t *>(ndfeatures2);
  int64_t *move = reinterpret_cast<int64_t *>(ndmove);
  float *result = reinterpret_cast<float *>(ndresult);
...
size_t load_hcpe(const std::string &filepath, std::ifstream &ifs,
                 bool use_average, const double eval_scale, int &len) {
  for (int p = 0; ifs; ++p) {
    HuffmanCodedPosAndEval hcpe;
    ...
    if (use_average) {
      ...
      if (ret.second) {
        auto &data = trainingData.emplace_back(
            hcpe.hcp, eval, make_result(hcpe.gameResult, hcpe.hcp.color()));
...
template <bool add>
inline void visits_to_proberbility(TrainingData &data,
                                   const std::vector<MoveVisits> &candidates,
  ...
  if (candidates.size() == 1) {
    ...
    const auto itr =
        std::max_element(candidates.begin(), candidates.end(),
                         [](const MoveVisits &a, const MoveVisits &b) {
    ...
    const float sum_visitNum =
        (float)std::accumulate(candidates.begin(), candidates.end(), 0,
                               [](int acc, const MoveVisits &move_visits) {
                                 return acc + move_visits.visitNum;
                               });
...
size_t __load_hcpe3(const std::string &filepath, bool use_average, double a,
  ...
  for (int p = 0; ifs; ++p) {
    ...
    for (int i = 0; i < hcpe3.moveNum; ++i) {
      ...
      if (moveInfo.candidateNum > 0) {
        ...
        ifs.read((char *)candidates.data(),
                 sizeof(MoveVisits) * moveInfo.candidateNum);
        ...
        if (use_average) {
          ...
          if (ret.second) {
            auto &data = trainingData.emplace_back(
                hcp, eval, make_result(hcpe3.result, pos.turn()));
...
// 重複データは平均化する
void __hcpe3_decode_with_value(const size_t len, char *ndindex,
                               char *ndfeatures1, char *ndfeatures2,
                               char *ndprobability, char *ndresult,
                               char *ndvalue, char *ndlegal_moves) {
  ...
  if (hcpe3) {
    for (int p = 0; ifs; ++p) {
      ...
      for (int i = 0; i < hcpe3.moveNum; ++i) {
        ...
        // candidateNum==0の手は読み飛ばす
        if (moveInfo.candidateNum > 0) {
          ifs.seekg(sizeof(MoveVisits) * moveInfo.candidateNum,
                    std::ios_base::cur);
          // 詰みは除く
