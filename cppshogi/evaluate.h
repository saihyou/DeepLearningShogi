...
// todo: 持ち駒 0
// の位置を詰めてテーブルを少しでも小さくする。(キャッシュに少しは乗りやすい?)
...
inline EvalIndex kppIndexBeginToOpponentBegin(const EvalIndex indexBegin) {
  switch (indexBegin) {
    ...
    case e_hand_lance:
      return f_hand_lance;
    case f_hand_knight:
      return e_hand_knight;
    case e_hand_knight:
      return f_hand_knight;
    case f_hand_silver:
      return e_hand_silver;
    case e_hand_silver:
      return f_hand_silver;
    case f_hand_gold:
      return e_hand_gold;
    case e_hand_gold:
      return f_hand_gold;
    case f_hand_bishop:
      return e_hand_bishop;
    case e_hand_bishop:
      return f_hand_bishop;
    case f_hand_rook:
      return e_hand_rook;
    case e_hand_rook:
      return f_hand_rook;
...
inline EvalIndex kppIndexToOpponentIndex(const EvalIndex index,
                                         const EvalIndex indexBegin,
                                         const EvalIndex opponentBegin) {
  return opponentBegin +
         (index < fe_hand_end
              ? index - indexBegin
              : (EvalIndex)inverse((Square)(index - indexBegin)));
}
inline EvalIndex kppIndexToOpponentIndex(const EvalIndex index,
                                         const EvalIndex indexBegin) {
  return kppIndexToOpponentIndex(index, indexBegin,
                                 kppIndexBeginToOpponentBegin(indexBegin));
}
inline EvalIndex kppIndexToOpponentIndex(const EvalIndex index) {
...
inline std::array<Tl, 2> operator+=(std::array<Tl, 2>& lhs,
                                    const std::array<Tr, 2>& rhs) {
  lhs[0] += rhs[0];
  lhs[1] += rhs[1];
  return lhs;
}
...
template <typename EvalElementType>
struct EvaluatorBase {
  ...
  // 先頭さえ分かれば良いので要素数1で良い。
  EvalElementType* oneArrayKPP(const u64 i) {
    return reinterpret_cast<EvalElementType*>(&kpps) + i;
  }
  EvalElementType* oneArrayKKP(const u64 i) {
    return reinterpret_cast<EvalElementType*>(&kkps) + i;
  }

  // todo: これらややこしいし汚いので使わないようにする。
  //       型によっては kkps_begin_index などの値が異なる。
  //       ただ、end - begin のサイズは型によらず一定。
  constexpr size_t kpps_end_index() const {
    return sizeof(kpps) / sizeof(EvalElementType);
  }
  constexpr size_t kkps_end_index() const {
    return sizeof(kkps) / sizeof(EvalElementType);
  }
  ...
  int64_t minKPPIndex(Square ksq, EvalIndex i, EvalIndex j) {
    ...
    if (i == j) return std::numeric_limits<int64_t>::max();
  ...
  int64_t minKKPIndex(Square ksq0, Square ksq1, EvalIndex i) {
    if (ksq0 == ksq1) return std::numeric_limits<int64_t>::max();

    if (i < fe_hand_end) {  // i 持ち駒
      if (i == kppIndexBegin(i)) return std::numeric_limits<int64_t>::max();
    } else {  // i 盤上
...
inline T0 atomicAdd(std::atomic<T0>& x, const T1& diff) {
  ...
  T0 desired = old + diff;
  while (!x.compare_exchange_weak(old, desired, std::memory_order_release,
...
template <typename T0, typename T1>
inline T0 atomicSub(std::atomic<T0>& x, const T1& diff) {
  return atomicAdd(x, -diff);
}

struct EvaluatorGradient
    : public EvaluatorBase<std::array<std::atomic<float>, 2>> {
...
struct Evaluator /*: public EvaluatorBase<EvalElementType>*/ {
  ...
  // 2GB を超えるファイルは Msys2 環境では std::ifstream では一度に read
  // 出来ず、分割して read する必要がある。
  static bool readEvalFile(const std::string& dirName) {
#define FOO(x)                                                      \
  {                                                                 \
    std::ifstream fs((addSlashIfNone(dirName) + #x ".bin").c_str(), \
                     std::ios::binary);                             \
    if (!fs) return false;                                          \
...
#define FOO(x)                                                      \
  {                                                                 \
    std::ofstream fs((addSlashIfNone(dirName) + #x ".bin").c_str(), \
                     std::ios::binary);                             \
    if (!fs) return false;                                          \
    auto end = (char*)x + sizeof(x##EvalElementType2);              \
    for (auto it = (char*)x; it < end; it += (1 << 30)) {           \
...
struct EvalSum {
  ...
  void encode() {
#if defined USE_AVX2_EVAL
    // EvalSum は atomic にコピーされるので key
    // が合っていればデータも合っている。
#else
