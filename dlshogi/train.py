...
def main(*argv):
    ...
    parser.add_argument(
        "--checkpoint", default="checkpoint-{epoch:03}.pth", help="checkpoint file name"
    )
    ...
    parser.add_argument(
        "--optimizer", default="SGD(momentum=0.9,nesterov=True)", help="optimizer"
    )
    ...
    parser.add_argument(
        "--val_lambda", type=float, default=0.333, help="regularization factor"
    )
    ...
    if args.lr_scheduler:
        if args.lr_scheduler[-1] != ")":
            args.lr_scheduler += "()"
        scheduler = eval(
            "optim.lr_scheduler." + args.lr_scheduler.replace("(", "(optimizer,")
        )
    if args.use_swa:
        logging.info(
        ...
        ema_avg = (
            lambda averaged_model_parameter, model_parameter, num_averaged: ema_a
            * averaged_model_parameter
            + ema_b * model_parameter
        )
    ...
    logging.info(
        "optimizer {}".format(re.sub(" +", " ", str(optimizer).replace("\n", "")))
    )

    logging.info("Reading training data")
    train_len, actual_len = Hcpe3DataLoader.load_files(
        args.train_data, args.use_average, args.use_evalfix, args.temperature
    )
    ...
    for e in range(args.epoch):
        ...
        for x1, x2, t1, t2, value, legal_moves in train_dataloader:
            ...
            if args.clip_grad_max_norm:
                ...
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), args.clip_grad_max_norm
                )
            ...
            if (
                args.use_swa
                and epoch >= args.swa_start_epoch
                and t % args.swa_freq == 0
            ):
            ...
            if t % eval_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    loss3 = bce_with_logits_loss(y2, value)
                    loss = (
                        loss1 + (1 - args.val_lambda) * loss2 + args.val_lambda * loss3
                    )
        ...
        logging.info(
            "epoch = {}, steps = {}, train loss avr = {:.07f}, {:.07f}, {:.07f}, {:.07f}, test loss = {:.07f}, {:.07f}, {:.07f}, {:.07f}, test accuracy = {:.07f}, {:.07f}, test entropy = {:.07f}, {:.07f}".format(
                epoch,
                t,
                sum_loss1_epoch / steps_epoch,
                sum_loss2_epoch / steps_epoch,
                sum_loss3_epoch / steps_epoch,
                sum_loss_epoch / steps_epoch,
                test_loss1,
                test_loss2,
                test_loss3,
                test_loss,
